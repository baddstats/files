...
cmh = function(B, start, jump.parm1, jump.parm2) {
  ...
  for (i in 2:(B + 1)) {
    ...
    num.r <- sum(log(dnorm(y, mean = mu.star, sd = sqrt(theta[i - 1, 2])))) + 
    ...
    den.r <- sum(log(dnorm(y, mean = theta[i - 1, 1], sd = sqrt(theta[i - 1, 2])))) + 
    ...
    # notice where we use the updated mu and where we do not
    # the sigmasq value just uses the previous iteration sigmasq
    num.r <- sum(log(dnorm(y, mean = theta[i, 1], sd = sqrt(sigmasq.star)))) + 
    ...
    den.r <- sum(log(dnorm(y, mean = theta[i, 1], sd = sqrt(theta[i - 1, 2])))) + 
